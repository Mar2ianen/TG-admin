...
impl TelegramGateway {
    ...
    pub async fn execute_checked(
        ...
        if let Some(key) = idempotency_key.clone()
            && let Some(storage) = self.idempotency_storage.as_ref()
        {
            ...
            if replayed {
                match effect.status.as_str() {
                    EXTERNAL_EFFECT_STATUS_COMPLETED => {
                        ...
                        let result = serde_json::from_str::<TelegramResult>(result_json).map_err(
                            |error| {
                                TelegramError::new(
                                    operation,
                                    TelegramErrorKind::Internal,
        ...
        let result = match self.transport.execute(request).await {
            ...
            Err(error) => {
                if let Some(key) = idempotency_key.as_ref()
                    && let Some(storage) = self.idempotency_storage.as_ref()
                    && let Ok(connection) = storage.open()
                    && let Ok(error_json) = serde_json::to_string(&error)
                {
                    let now = Utc::now().to_rfc3339();
