...
## Работа с контентом (Шаблоны)

Для соблюдения чистоты кода **строго запрещено хардкодить тексты сообщений внутри `.rhai` скриптов**.

Для управления контентом используйте систему шаблонов:
1. Положите шаблон в `bundled_templates/<category>/<name>.txt`.
2. Используйте `load_template("<name>")` для загрузки.
3. Используйте `render_auto("<name>")` для автоматической подстановки контекста (имя пользователя, дата, чат).

Если админу нужно изменить текст, он создаст файл в `templates/` (с тем же именем), который перекроет встроенный шаблон.

Пример правильного кода:
```rhai
// Неправильно: let text = "Привет, " + name;
// Правильно:
let text = render_auto("welcome");
tg_send_message(event.chat.id, text);
```

## Что script не должен делать

Script не должен:
- **Содержать жестко закодированные тексты сообщений** (используйте шаблоны).
...
