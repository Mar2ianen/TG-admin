...
## Step 1. Собрать composition root

Сейчас `Application` — lifecycle shell. Для MVP нужен отдельный `Runtime` как явный execution graph.
...
## Step 3. Подключить реальный Telegram transport

Сейчас `src/tg.rs` уже дает typed contract, но transport по умолчанию все еще `noop`.

Нужно сделать:

- transport поверх `teloxide-core`
...
## Step 4. Собрать ingress pipeline
